...
def test_corte_control_varias_sucursales():
    ...
    assert sucursal_1["mayor_producto"] == "A"
    assert sucursal_1["mayor_importe"] == 200
    assert sucursal_1["menor_producto"] == "B"
    assert sucursal_1["menor_importe"] == 50

    assert sucursal_2["sucursal"] == "2"
    assert sucursal_2["total_unidades"] == 7
    assert sucursal_2["mayor_producto"] == "D"
    assert sucursal_2["mayor_importe"] == 80
    assert sucursal_2["menor_producto"] == "C"
    assert sucursal_2["menor_importe"] == 30


def test_corte_control_sin_datos():
    filas = []

    resultado = procesar_corte_control(filas)

    assert resultado["sucursales"] == []
    assert resultado["totales_generales"]["cantidad_sucursales"] == 0
    assert resultado["totales_generales"]["total_importe"] == 0


def test_cantidad_invalida():
    filas = [
        {"PRSUC": "1", "PRCOD": "A", "PRCANT": "abc", "PRPRE": "100"},
    ]

    with pytest.raises(ValueError):
        procesar_corte_control(filas)


def test_precio_invalido():
    filas = [
        {"PRSUC": "1", "PRCOD": "A", "PRCANT": "2", "PRPRE": "cien"},
    ]
